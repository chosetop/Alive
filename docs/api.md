...
本文档描述当前已实现的接口。标签公开聚合、媒体上传凭证与管理端媒资接口仍以实现进度为准。
...
| `page` / `page_size` | 见 1.3 |
| `status` | 按状态筛，值必须在枚举内 |
| `q` | 按文本搜 `title`、`slug`、`summary`，大小写不敏感 |

`?status=stauts`（拼错）返回 **400** 且 `fields.status: "must be one of draft, published, archived"`，不是空列表。返回空列表会被读成「没有草稿」，而实际是「你拼错了」。

`q` 与 `status` 是**交集**，不是并集。两者都给时，只返回同时满足的条目。

`q` 的三条规则：

- **不搜正文。** 长文里的常见词会把真正以该词命名的文章挤掉，而目录的职责是找到一篇已知文章，不是全文检索。
- **空白等于没传。** 边打边搜的输入框每次击键都会发一次请求，包括清空的那一次；把空串当成「搜索空字符串」会在用户期待目录回来的那一刻把它清空。
- **`%`、`_`、`\` 按字面处理。** 它们是 LIKE 的模式字符，不转义的话打一个 `_` 会返回整个目录，而「读完了 80% 的书」这种标题根本搜不到。
